...
func readConfig() (string, string, error) {
	jsonFile, err := os.Open("randomphototidbyt.config.json")
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	for i < 1 {
		...
		cmi = r1.Intn(mic - 1)
		log.Println("(portrait, try again) Chosen Media Index is: " + strconv.Itoa(cmi))
	}
	log.Println("Chosen Media Url: " + mediaUrl)
